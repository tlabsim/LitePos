...
(function () {
    ...
    function renderCustomersTable() {
        const els = _getEls();
        const db = _getDb();
        const tbody = _getEl('customers-table-body');
        if (!tbody) {
            console.error('[customers.js] tbody NOT FOUND!');
            return;
        }
        tbody.innerHTML = '';
        const query = (_getEl('customer-search') && _getEl('customer-search').value || '').trim().toLowerCase();
    ...
    function loadCustomerToForm(id) {
        ...
        if (!c) return;
        if (_getEl('customer-edit-name')) _getEl('customer-edit-name').value = c.name;
    ...
    function clearCustomerForm() {
        const els = _getEls();
        if (_getEl('customer-edit-name')) _getEl('customer-edit-name').value = '';
    ...
    function saveCustomerFromForm() {
        const els = _getEls();
        const db = _getDb();
        const name = (_getEl('customer-edit-name') && _getEl('customer-edit-name').value || '').trim();
    ...
    function findCustomerFromInput() {
        const els = _getEls();
        const db = _getDb();
        const phone = (_getEl('sale-customer-phone') && _getEl('sale-customer-phone').value || '').trim();
        if (!phone) {
        ...
        if (customer) {
            ...
            if (_getEl('sale-customer-name')) _getEl('sale-customer-name').value = customer.name;
            ...
            if (_getEl('sale-customer-name')) _getEl('sale-customer-name').value = '';
    ...
    function setCurrentCustomer(customer) {
        ...
        if (!customer) {
            const badge = _getEl('summary-customer-badge') || document.getElementById('summary-customer-badge');
            if (badge) badge.textContent = 'Walk-in';
            if (_getEl('sale-customer-phone')) _getEl('sale-customer-phone').value = '';
            if (_getEl('sale-customer-name')) _getEl('sale-customer-name').value = '';
            if (_getEl('quick-customer-address')) _getEl('quick-customer-address').value = '';
        } else {
            ...
            if (_getEl('sale-customer-phone')) _getEl('sale-customer-phone').value = customer.phone || '';
            if (_getEl('sale-customer-name')) _getEl('sale-customer-name').value = customer.name || '';
    ...
    function saveQuickCustomer() {
        ...
        const name = (_getEl('sale-customer-name') && _getEl('sale-customer-name').value || '').trim() || 'Customer';
        const address = (_getEl('quick-customer-address') && _getEl('quick-customer-address').value || '').trim();
        const notes = (_getEl('quick-customer-notes') && _getEl('quick-customer-notes').value || '').trim();
        ...
        if (existing) {
            ...
            existing = { id: 'c' + (db.customers.length + 1), name, phone, address, notes, lastSaleAt: null, lastSaleTotal: 0 };
        ...
        if (_getEl('quick-customer-address')) _getEl('quick-customer-address').value = '';
        if (_getEl('quick-customer-notes')) _getEl('quick-customer-notes').value = '';
    ...
    function focusCustomerPhone() {
        const els = _getEls();
        if (_getEl('sale-customer-phone')) { _getEl('sale-customer-phone').focus(); _getEl('sale-customer-phone').select(); }
    }

    function renderCustomerOverlay() {
        const els = _getEls();
        const db = _getDb();
        const overlayBody = _getById('customer-overlay-body');
        const overlay = _getById('customer-overlay');
        if (!overlayBody || !overlay) return;
        const q = (_getEl('sale-customer-phone') && _getEl('sale-customer-phone').value || '').trim().toLowerCase();
        ...
        customers.forEach(c => {
            ...
            tr.addEventListener('click', () => {
                ...
                if (_getEl('sale-customer-phone')) _getEl('sale-customer-phone').value = c.phone || '';
                if (_getEl('sale-customer-name')) _getEl('sale-customer-name').value = c.name || '';
            ...
            const tdAddress = document.createElement('td'); tdAddress.textContent = c.address || '—'; tdAddress.style.fontSize = '12px'; tdAddress.style.color = 'var(--text-soft)'; tr.appendChild(tdAddress);
    ...
    document.addEventListener('input', (ev) => {
        const els = _getEls();
        if (!_getEl('sale-customer-phone')) return;
        if (ev.target !== _getEl('sale-customer-phone')) return;
    ...
    document.addEventListener('click', (ev) => {
        const els = _getEls();
        const overlay = _getById('customer-overlay');
        const input = _getEl('sale-customer-phone');
        if (!overlay) return;
